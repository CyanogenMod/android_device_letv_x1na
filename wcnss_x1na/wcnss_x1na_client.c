...
static const char x1na_mac_prefix[] = { 0x84, 0x73, 0x03 };
...
int wcnss_qmi_get_wlan_address(unsigned char *pBdAddr)
{
    ...
    if (!success) {
        memcpy(pBdAddr, x1na_mac_prefix, sizeof(x1na_mac_prefix));
        ...
        for (i = sizeof(x1na_mac_prefix) / sizeof(x1na_mac_prefix[0]); i < MAC_ADDR_SIZE; i++) {
            pBdAddr[i] = rand() % 255;
        }
